...
Program akan terus membaca teks tersebut dan menguwmpulkannya ke dalam sebuah struktur data *vector* bernama `http_request` melalui metode `.collect()`. 
...
Jika request tersebut meminta path root (`GET / HTTP/1.1`), server akan mengvembalikan file `hello.html` dengan status 200 OK. Namun, jika path yang diminta berbeda, server akan masuk ke blok `else` dan mengembalikan halaman error `404.html` dengan status 404 NOT FOUND.
...
Pemisahan (*split*a) struktur ini membuat kode jauh lebih ringkas, bersih, dan mempermudah penambahan halaman baru di masa depan karena logika pengiriman respons terpusat di satu tempat.

## Commit 4 Reflection notes

Pada milestone ini, saya mengganti struktur kontrol `if-else` menjadi `match` untuk menangani rute baru, yaitu `/sleep`. 
